<template>
  <v-toolbar density="compact" color="primary">
    <v-btn icon @click="$router.go(-1)">
      <v-icon>mdi-arrow-left</v-icon>
    </v-btn>
    <v-toolbar-title>Start Match</v-toolbar-title>
  </v-toolbar>

  <v-container class="d-flex flex-column ga-3">

    <p class="text-h5">Team 1</p>
    <v-autocomplete label="Player 1" variant="outlined"
      :items="['California', 'Colorado', 'Florida', 'Georgia', 'Texas', 'Wyoming']"></v-autocomplete>
    <v-autocomplete label="Player 2" variant="outlined"
      :items="['California', 'Colorado', 'Florida', 'Georgia', 'Texas', 'Wyoming']"></v-autocomplete>

      <p class="text-h5">Team 2</p>
    <v-autocomplete label="Player 1" variant="outlined"
      :items="['California', 'Colorado', 'Florida', 'Georgia', 'Texas', 'Wyoming']"></v-autocomplete>
    <v-autocomplete label="Player 2" variant="outlined"
      :items="['California', 'Colorado', 'Florida', 'Georgia', 'Texas', 'Wyoming']"></v-autocomplete>

    <v-btn block text="Team 1 Win" variant="outlined" @click="onStart"></v-btn>
    <v-btn block text="Team 2 Win" variant="outlined" @click="onStart"></v-btn>
    <v-spacer></v-spacer>
    <v-btn block color="primary" text="Start" variant="flat" @click="onStart"></v-btn>

  </v-container>


</template>

<script setup>
const router = useRouter()

const onStart = () => {
  router.push({ path: '/matches', replace: true })
}
</script>

<style lang="css" scoped></style>